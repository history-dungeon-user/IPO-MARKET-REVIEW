...
def extract_registration(text):
    ...
    # ※ 증권종류 셀에 공백이 들어가는 표기가 있다("기명식 보통주"). 이를 `[가-힣]{2,12}`로
    #    잡으면 '기명식'까지만 먹고 다음 `|`에 도달하지 못해 표 전체가 통째로 미매칭된다.
    #    (네오사피엔스 발행조건확정 사례: 확정 10,000인데 이번 파싱이 실패해 이전 신고서의
    #     밴드값 13,800이 그대로 남았음.) → 종류명에 내부 공백을 허용한다(비탐욕).
    _ov = list(re.finditer(
        r"증권의\s*종류\s*\|\s*증권\s*수량[^\n]*\n\s*([가-힣][가-힣 ]{1,15}?)\s*\|\s*([\d,]+)\s*\|\s*"
        r"[^|]+\|\s*([\d,]+)\s*\|\s*([\d,]+)\s*\|", text))
    ...
    # 확정본 전용 — 확정공모가(문구). 단, 실제 정본은 공모개요 표의 '모집가액'이며
    # 마스터 빌드( 발행조건확정 → 확정공모가 = 모집가액 )에서 이 값으로 덮어쓴다.
    # 문구 정규식은 위험 서술문("…하단 N원에 미달")의 밴드값을 오인할 수 있으므로,
    # '…을 N원으로 결정/확정' 또는 '확정공모가액인 N원'만 인정한다.
