...
def extract_underwriters(t):
    ...
    rows = []
    # 증권종류 칸은 신고서마다 '기명식보통주' / '보통주' / '기명식 보통주' 등으로 다르다.
    # '기명식'을 하드코딩하면 '보통주'만 쓰는 최초 신고서(니어스랩 등)가 통째로 빠진다.
    #   → '주'로 끝나는 짧은 칸이면 무엇이든 허용. 인수방법도 총액/잔액 모두 허용.
    for m in re.finditer(
        r"(공동대표주관회사|대표주관회사|공동주관회사|인수회사|인수단|대표)\s*\|\s*"
        r"([가-힣A-Za-z]+증권)\s*\|\s*[^|]{0,15}?주\s*\|\s*([\d,]+)\s*\|\s*([\d,]+)\s*\|\s*([\d,]+)\s*\|\s*(?:총액|잔액)?인수", t):
